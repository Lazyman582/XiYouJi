    ...
    public class DepthOfField : PostEffectsBase {
        ...
        private void WriteCoc ( RenderTexture fromTo, bool fgDilate) {
            ...
            if (nearBlur && fgDilate) {
                ...
                // Unity 2022 warns/errors when Graphics.Blit reads and writes the same RenderTexture.
                // Use a temporary buffer so the legacy effect remains compatible with newer Unity versions.
                RenderTexture cocResult = RenderTexture.GetTemporary (fromTo.width, fromTo.height, 0, fromTo.format);
                Graphics.Blit (fromTo, cocResult, dofHdrMaterial, 13);
                // Pass 13 has ColorMask A, so the original RGB (including the warm grading
                // from the other image effects) stays intact while only CoC alpha is written.
                Graphics.Blit (cocResult, fromTo, dofHdrMaterial, 13);
                RenderTexture.ReleaseTemporary (cocResult);
                RenderTexture.ReleaseTemporary(temp2);
            }
            else {
                ...
                fromTo.MarkRestoreExpected(); // only touching alpha channel, RT restore expected
                RenderTexture cocResult = RenderTexture.GetTemporary (fromTo.width, fromTo.height, 0, fromTo.format);
                Graphics.Blit (fromTo, cocResult, dofHdrMaterial, 0);
                // Pass 0 is the CoC writer; keep the source/destination separate to avoid
                // Unity 2022's in-place Blit warning while preserving the original RGB.
                Graphics.Blit (cocResult, fromTo, dofHdrMaterial, 0);
                RenderTexture.ReleaseTemporary (cocResult);
            }
        }
